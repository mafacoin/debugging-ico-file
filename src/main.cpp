...
#if defined(NDEBUG)
# error "MafaCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x58747a240f16e0ce0c7f142cf1081953703e103a20f4b34b236b2d6b8b9fc387");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MafaCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MafaCoin Signed Message:\n";
...
{
    // MafaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // MafaCoin: 10k blocks

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // MafaCoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // MafaCoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MafaCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MafaCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x58747a240f16e0ce0c7f142cf1081953703e103a20f4b34b236b2d6b8b9fc387");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 05/Feb/2018 Stock Plunge as Sell-Off Enters 2nd Week";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 69 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044739abe61e2e5b7f23e3624e299d103182a6b85a0ab49b8878a867432cdc14f9adb3edbc25ae13ee6dfdb0e460f0d265584cdf30656304369184f743f6ec0e76") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517892260;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1476649;
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x1db152b6442b7c644140ba032e1be71c5839b5a62d7d7f1c289194aacf4734d2"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xff, 0xc9, 0xb8, 0xdd }; // MafaCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MafaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MafaCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MafaCoinMiner(CWallet *pwallet)
{
    printf("MafaCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mafacoin-miner");
    ...
    try { loop {
        ...
        printf("Running MafaCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MafaCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MafaCoinMiner, pwallet));
}
